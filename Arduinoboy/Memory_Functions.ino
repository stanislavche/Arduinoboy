// Memory_Functions.ino — S_TN fork

boolean checkMemory() {
  for (int m = 0; m < 4; m++) {
    if (EEPROM.read(MEM_CHECK + m) != defaultMemoryMap[MEM_CHECK + m]) return false;
  }
  return true;
}
...
void initMemory(boolean reinit) {
  if (!alwaysUseDefaultSettings) {
    if (reinit || !checkMemory()) {
      for (int m = MEM_MAX; m >= 0; m--) EEPROM.write(m, defaultMemoryMap[m]);
    }
    loadMemory();
  } else {
...
void loadMemory() {
  for (int m = MEM_MAX; m >= 0; m--) memory[m] = EEPROM.read(m);
  changeTasks();
}

void saveMemory() {
  for (int m = MEM_MAX - 1; m >= 0; m--) EEPROM.write(m, memory[m]);
  changeTasks();
}
